...
// Check if Super Admin (user_id = 0) - profile updates disabled for Super Admin
$isSuperAdmin = isset($_SESSION['role']) && $_SESSION['role'] === ROLE_SUPER_ADMIN && ($userId === 0 || $userId === '0');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if (!CSRF::validateToken($_POST['csrf_token'] ?? '')) {
        ...
        // Super Admin cannot update profile (no database record)
        if ($isSuperAdmin) {
            $message = 'Profile updates are disabled for Super Admin (Development Mode). Changes would be lost on logout.';
            $messageType = 'warning';
        } else {
            try {
                switch ($action) {
                    ...
                    case 'remove_photo':
                        ...
                        try {
                            ...
                            if ($oldPhoto) {
                                $absolutePhotoPath = __DIR__ . '/../' . ltrim($oldPhoto, '/');
...
// Note: $isSuperAdmin is already defined above for profile update handling

if (isset($isSuperAdmin) && $isSuperAdmin) {
    // Super Admin profile - use session data only
    $user = [
        'id' => 0,
        'username' => $_SESSION['username'] ?? 'superadmin',
        'full_name' => $_SESSION['full_name'] ?? 'Super Admin (Development)',
...
<div class="container-fluid">
    ...
    <div class="dashboard-grid">
        ...
        <div class="dashboard-card">
            ...
            <?php if ($isSuperAdmin): ?>
                <div class="alert alert-warning" style="margin-top: 20px;">
                    <strong>⚠️ Super Admin Mode (Development):</strong> Profile photo uploads are disabled for Super Admin.
                </div>
            <?php elseif ($hasProfilePhoto): ?>
                ...
                <?php if ($photoPath && $absolutePhotoPath && file_exists($absolutePhotoPath)): ?>
                    <form method="POST" style="margin-top: 10px;" onsubmit="return confirm('Are you sure you want to remove your profile photo?');">
                        <?php echo CSRF::getTokenField(); ?>
                        <input type="hidden" name="action" value="remove_photo">
                        <button type="submit" class="btn btn-outline btn-danger" style="width: 100%;">
                            🗑️ Remove Photo
                        </button>
                    </form>
                <?php endif; ?>
        ...
        <div class="dashboard-card">
            ...
            <?php if ($isSuperAdmin): ?>
                <div class="alert alert-warning" style="margin-bottom: 20px;">
                    <strong>⚠️ Super Admin Mode (Development):</strong> Profile updates are disabled for Super Admin. Changes would be lost on logout since Super Admin doesn't have a database record.
                </div>
            <?php endif; ?>
            ...
            <?php endif; ?>
            <form method="POST" class="form-grid-compact" <?php echo $isSuperAdmin ? 'onsubmit="return false;"' : ''; ?>>
                <?php echo CSRF::getTokenField(); ?>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="full_name" class="form-control" 
                           ...
                           <?php echo $isSuperAdmin ? 'readonly' : 'required'; ?>>
                </div>
                ...
                <?php if ($hasNewColumns): ?>
                    <div class="form-group">
                        ...
                        <input type="tel" name="phone_number" class="form-control" 
                               ...
                               placeholder="+233 555 123 456" <?php echo $isSuperAdmin ? 'readonly' : ''; ?>>
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label">Date of Birth</label>
                        <input type="date" name="date_of_birth" class="form-control" 
                               value="<?php echo e($user['date_of_birth'] ?? ''); ?>" <?php echo $isSuperAdmin ? 'readonly' : ''; ?>>
                    </div>
                    
                    <div class="form-group full-width">
                        <label class="form-label">Bio</label>
                        <textarea name="bio" class="form-control" rows="3" 
                                  placeholder="Tell us about yourself" <?php echo $isSuperAdmin ? 'readonly' : ''; ?>><?php echo e($user['bio'] ?? ''); ?></textarea>
                    </div>
                    
                    <div class="form-group full-width">
                        <label class="form-label">Address</label>
                        <textarea name="address" class="form-control" rows="2" 
                                  placeholder="Street address" <?php echo $isSuperAdmin ? 'readonly' : ''; ?>><?php echo e($user['address'] ?? ''); ?></textarea>
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label">City</label>
                        <input type="text" name="city" class="form-control" 
                               value="<?php echo e($user['city'] ?? ''); ?>" <?php echo $isSuperAdmin ? 'readonly' : ''; ?>>
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label">Country</label>
                        <input type="text" name="country" class="form-control" 
                               value="<?php echo e($user['country'] ?? 'Ghana'); ?>" <?php echo $isSuperAdmin ? 'readonly' : ''; ?>>
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label">Postal Code</label>
                        <input type="text" name="postal_code" class="form-control" 
                               value="<?php echo e($user['postal_code'] ?? ''); ?>" <?php echo $isSuperAdmin ? 'readonly' : ''; ?>>
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label">Emergency Contact Name</label>
                        <input type="text" name="emergency_contact_name" class="form-control" 
                               value="<?php echo e($user['emergency_contact_name'] ?? ''); ?>" <?php echo $isSuperAdmin ? 'readonly' : ''; ?>>
                    </div>
                    
                    <div class="form-group">
                        <label class="form-label">Emergency Contact Phone</label>
                        <input type="tel" name="emergency_contact_phone" class="form-control" 
                               value="<?php echo e($user['emergency_contact_phone'] ?? ''); ?>" <?php echo $isSuperAdmin ? 'readonly' : ''; ?>>
                    </div>
                <?php endif; ?>
                
                <div class="form-group full-width">
                    <button type="submit" class="btn btn-primary" <?php echo $isSuperAdmin ? 'disabled' : ''; ?>>
                        <?php echo $isSuperAdmin ? 'Update Profile (Disabled for Super Admin)' : 'Update Profile'; ?>
                    </button>
                </div>
            </form>
        </div>
        ...
        <div class="dashboard-card">
            ...
            <?php if ($isSuperAdmin): ?>
                <div class="alert alert-warning" style="margin-bottom: 20px;">
                    <strong>⚠️ Super Admin Mode (Development):</strong> Password changes are disabled for Super Admin. Super Admin uses development credentials configured in <code>config/super-admin.php</code>.
                </div>
            <?php endif; ?>
            <form method="POST" <?php echo $isSuperAdmin ? 'onsubmit="return false;"' : ''; ?>>
                <?php echo CSRF::getTokenField(); ?>
                ...
                <div class="form-group">
                    ...
                    <input type="password" name="current_password" class="form-control" 
                           <?php echo $isSuperAdmin ? 'readonly disabled' : 'required'; ?>>
                </div>
                
                <div class="form-group">
                    <label class="form-label">New Password *</label>
                    <input type="password" name="new_password" class="form-control" 
                           <?php echo $isSuperAdmin ? 'readonly disabled' : 'required minlength="8"'; ?>>
                ...
                <div class="form-group">
                    <button type="submit" class="btn btn-primary" style="width: 100%;" 
                        ...
                        <?php echo $isSuperAdmin ? 'Change Password (Disabled for Super Admin)' : 'Change Password'; ?>
                    </button>
                </div>
            </form>
        </div>
        ...
        <div class="dashboard-card">
            ...
            <div style="margin-top: 20px;">
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px; border: 1px solid var(--border); border-radius: 6px; margin-bottom: 10px;">
                    ...
                    <?php else: ?>
                        <a href="<?php echo api_url('social-auth.php', ['action' => 'google_auth']); ?>" class="btn btn-sm btn-primary">Connect</a>
                    <?php endif; ?>
                </div>
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px; border: 1px solid var(--border); border-radius: 6px; margin-bottom: 10px;">
                    ...
                    <?php else: ?>
                        <a href="<?php echo api_url('social-auth.php', ['action' => 'facebook_auth']); ?>" class="btn btn-sm btn-primary">Connect</a>
                    <?php endif; ?>
                </div>
